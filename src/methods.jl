...
Base.rem(x::Integer, ::Type{T}) where T<:EmulatedSigned   = reinterpret(T, ((x % storagetypeof(T)) << wastedbits(T)) >> wastedbits(T))
Base.rem(x::Integer, ::Type{T}) where T<:EmulatedUnsigned = reinterpret(T, x % storagetypeof(T) & maxvalue(T))
...
Base.string(x::EmulatedInteger; base::Integer=10, pad::Integer=1) = string(x[]; base, pad)

function Base.exponent(x::EmulatedInteger)
    iszero(x) && throw(DomainError(x, "cannot be zero"))
    storage = abs(x[])
    return 8sizeof(storage) - leading_zeros(storage) - 1
end
