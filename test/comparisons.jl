@testset "float comparisons" begin
    ...
    sources = (Int1, UInt1, Int3, UInt3, Int31, UInt31, Int63, UInt63,
               Int65, UInt65, Int129, UInt129, Int3_256, UInt3_256,
               Int20_24, UInt20_24, Int63_256, UInt63_256, Int65_256, UInt65_256,
               Int128_256, UInt128_256, Int12, UInt12, UInt24_64, Int25, UInt25,
               Int53, UInt53, Int54, UInt54, Int3_128, UInt3_128)
    for Source in sources
        low, high = BigInt(typemin(Source)), BigInt(typemax(Source))
        values = [low, low + 1, high - 1, high, big(0), big(1)]
        ...
        unique!(values)
        for Float in (Float16, Float32, Float64, BigFloat), value in values
            x = Source(value)
            rounded = Float(value)
            ...
            for y in floats, op in operators
                @test (@inferred op(x, y)) === op(value, y)
                @test (@inferred op(y, x)) === op(y, value)
            end
            for y in floats
                if isequal(value, y)
                    @test hash(x) == hash(y)
